...
size_t bfs(const Graph& G, Vertex u, std::vector<Vertex>& P, std::vector<size_t>& D) {
    ...
    // Start init of first vertex and queue
    que.push(u);
    P[u] = ROOT;
    D[u] = 0;
    ...
    // BFC algorithm
    while(!que.empty()) {
        auto curVertex = que.front();
        que.pop();

        // Check of neighbors
        for(auto child : G[curVertex]) {
            // Control, that vertex isn't visited
            if(P[child] == NO_VERTEX) {
                D[child] = D[curVertex] + 1;
                P[child] = curVertex;
