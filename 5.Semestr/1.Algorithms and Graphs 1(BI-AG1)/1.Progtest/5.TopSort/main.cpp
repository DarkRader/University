...
bool findCycle(Vertex v, const Graph& G, std::vector<State>& states, std::vector<Vertex>& cycle) {
    states[v] = State::VISITING;

    for (Vertex neighbor : G[v]) {
        if (states[neighbor] == State::UNVISITED) {
            if (findCycle(neighbor, G, states, cycle)) {
                cycle.push_back(v);
                return true;
            }
...
std::pair<bool, std::vector<Vertex>> topsort(const Graph& G) {
    ...
    vector<size_t> level(G.vertices(), 0);
    ...
    while (!que.empty()) {
        ...
        for (const auto& child : G[curVertex]) {
            level[child]--;
            if(level[child] == 0) {
                que.push(child);
            }
    ...
    if(result.size() != G.vertices()) {
        ...
        std::vector<bool> in_stack(G.vertices(), false);
        Vertex start_vertex = Vertex{0};  // Initialize with an arbitrary vertex

        for (Vertex v : G) {
            if (!visited[v] && findCycle(v, G, visited, in_stack, cycle, start_vertex)) {
                break;
            }
        }
