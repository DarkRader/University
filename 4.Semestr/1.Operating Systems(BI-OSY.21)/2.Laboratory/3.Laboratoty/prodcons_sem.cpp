/********************************************************************************
  ...
  - There are 2 errors in the code. => Try to find then. 
    ...
    1) Why there is deadlock?
       => wrong locking

    2) Why process terminates with "Segmentation fault"
       => wrong semaphore initialization
...
public: 
  BufferClass() 
  { 
    sem_init ( &sem_free, 0, BUFFER_SIZE );                  // <--- #2                  
    sem_init ( &sem_full, 0, 0 );                            // <--- #2                     
  }
